<template>
  <div class="flex flex-col flex-1 items-center">
    ...
    <div v-if="route.query.preview" class="text-white p-4 bg-weather-secondary w-full text-center">
      <p>現在この都市をプレビューしています。この都市のトラッキングを開始するには、「+」アイコンをクリックしてください。
      </p>
    </div>
...
const getWeatherData = async () => {
  try {
    const weatherData = await axios.get(
      `https://api.openweathermap.org/data/3.0/onecall?lat=${route.query.lat}&lon=${route.query.lng}&exclude={part}&appid=8a12db13bdbcc785aa243b57fff9955a&units=metric`
    );
